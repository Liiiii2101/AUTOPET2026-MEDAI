...
from nnunetv2.training.dataloading.data_loader_3d_interactive import nnUNetDataLoader3DInteractive
...
from nnunetv2.training.nnUNetTrainer.curriculum.MyCustomCurriculumTrainerSegPre import MyCustomCurriculumTrainerSegPre

class MyCustomCurriculumTrainerSegPreSkel(MyCustomCurriculumTrainerSegPre):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.weight_srec = 0.8 # This is the default value, you can change it if you want
    ...
    def get_dataloaders(self):
        ...
        if dim == 2:
            dl_tr = nnUNetDataLoader3DInteractive(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader3DInteractive(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
    ...
    def get_training_transforms(
        ...
        print('rotation for DA:', rotation_for_DA)
        transforms = []
